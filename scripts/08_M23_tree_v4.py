#!/usr/bin/env python3
"""M23 phylogeny v4 — CwlT removed.

Rationale: CwlT's C-terminal peptidase belongs to the NlpC/P60 cysteine-peptidase
family (Pfam PF00877), NOT Pfam Peptidase_M23 (PF01551), so it should never have
been in an M23 phylogeny. CwlP (true M23-family) is retained as the only
published SLT+M23 architectural reference.

Derived from scripts/07_M23_tree_v3.py with one structural change (CwlT removed)
and updated title/footnote text.
"""
...
TREE = ROOT/"results/07_M23_phylogeny/M23_iqtree_v4.treefile"
OUT  = ROOT/"figures"

CAT_COLORS = {
    "PL25":              "#D7191C",
    "Reference_SLT_M23": "#9C27B0",   # CwlP only — the one true SLT+M23 reference
    "Reference":         "#7B1FA2",
    "Phage_M23":         "#FF7F00",
...
# Build category lookup from seed v3 (same input, minus CwlT — categories unchanged)
tax_map = {}
with open(ROOT/"results/07_M23_phylogeny/M23_seedset_v3.faa") as f:
    for line in f:
        ...
        ide = head.split()[0]
        if "CwlT" in ide:
            continue  # CwlT excluded from this tree
...
ax.set_title("M23 phylogeny v4 — PL25 plasmid SLT/M23 in context of CwlP (SP-β prophage)\n"
...
for name in ordered:
    if name in CAT_COLORS:
        ...
        if name == "Reference_SLT_M23": label = "Reference SLT/M23 (CwlP only)"
...
fig.savefig(OUT/"02d_M23_phylogeny_v4_CwlT_removed.pdf", bbox_inches="tight")
fig.savefig(OUT/"02d_M23_phylogeny_v4_CwlT_removed.svg", bbox_inches="tight")
fig.savefig(OUT/"02d_M23_phylogeny_v4_CwlT_removed.png", dpi=200, bbox_inches="tight")
print(f"✓ Saved 02d_M23_phylogeny_v4_CwlT_removed.{{pdf,svg,png}}")
